    ...
    <div id="rollback-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full mx-4">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-xl font-bold text-gray-900">確認退回此版本</h3>
            </div>
            <div class="mb-6 text-gray-600 space-y-2">
                <p>此操作將會：</p>
                <ul class="list-disc list-inside space-y-1 text-sm">
                    <li>將 <strong class="text-blue-600"><?= number_format($action_stats['update']) ?> 筆更新</strong>的商品還原為此次執行<strong>前</strong>的 price / size</li>
                    <li>刪除此次執行中<strong class="text-green-600">新增的 <?= number_format($action_stats['create']) ?> 筆</strong>商品</li>
                </ul>
                <p class="mt-3 text-sm text-red-600 font-medium">
                    <i class="fas fa-exclamation-triangle mr-1"></i>此操作不可復原，請確認後再執行。
                </p>
            </div>
            <div class="flex gap-3 justify-end">
                <button onclick="document.getElementById('rollback-modal').classList.add('hidden')"
                        class="px-5 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">
                    取消
                </button>
                <form method="POST" action="/admin/execution-history/rollback/<?= esc($execution['execution_id']) ?>">
                    <?= csrf_field() ?>
                    <button type="submit"
                            class="px-5 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-medium transition">
                        <i class="fas fa-undo mr-1"></i>確認退回
                    </button>
                </form>
            </div>
        </div>
